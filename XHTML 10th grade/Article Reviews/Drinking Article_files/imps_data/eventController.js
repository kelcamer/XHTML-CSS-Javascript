...
function mobileClickAssuarance(appId,adId,zid)
{
    if (confirm('Did you mean to click on this ad? Click OK to continue.') ) {
        return true;
    } else {
        var eventId = 4;
        var eventParams = encodeURI("appId="+appId+"&zid="+zid+"&adId="+adId+"&eventType="+eventId);
        setServerRequest(eventParams);        
        return false;
    }            
}