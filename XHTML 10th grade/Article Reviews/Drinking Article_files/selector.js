GEL.Element.Base.prototype.get= function(selection,filterFn) { 
	var _el= this.getElement(), _ret= new Array(), _n= null, i=0; 
	if(!_el) return; 
	_n= YAHOO.util.Selector.query(selection, _el); 
	if(typeof filterFn != 'function') return _n; 
	for(i=0;i<_n.length;i++) { 
		if( filterFn.call(_n[i]) )  
			_ret.push(_n[i]); 
	}
		
	return _ret; 
}
GEL.Element.Base.prototype.$= GEL.Element.Base.prototype.get;  
GEL.Element.Base.prototype.foreach= 
function(selector,callback){ 
	if(typeof callback != 'function') 
		throw new TypeError("callback must be a function reference");
	var _a= this.get(selector),i=0; 
	for(;i<_a.length;i++) 
		callback.call(_a[i]); 
}
GEL.Element.Base.prototype.map= function(selector,mapFn){ 
	if(typeof mapFn != 'function') 
		throw new TypeError("mapFn must be a function"); 
	var _ret= [], _a= this.get(selector), i=0,_el; 
	for(;i< _a.length; i++) { 
		_el= mapFn(_a[i]); 
		_ret.push( _el );  
	}
...
//if (YAHOO.env.ua.ie && ((!document.documentMode && YAHOO.env.ua.ie < 8))) { // rewrite class for IE (others use getAttribute('class')
